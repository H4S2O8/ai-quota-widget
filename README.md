...
2. **自动更新**：`remoteResource` 指向「最新 release 的 zip」，发一个新 release
   就等于推一次更新。注意两点——只有发了 release 才更新，push 到 main 不会；
   而且**已经装好的旧版本救不回来**，它存的是旧 URL，必须手动重装一次才进入
   自动更新的轨道。

   别填 GitHub 的 `tree/...` 网页地址，那个返回的是 HTML，不是 zip，装上之后
   会一直停在初始版本且不报错。
