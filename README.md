...
终端风排版：一行提示符、等宽对齐的列、方块进度条、底下一行状态。
三种尺寸各回答一个问题：

| 尺寸 | 回答 | 排法 |
| --- | --- | --- |
| 小 | 最该操心的那个还剩多少 | 大字是**最紧张的那个窗口**（不是账户的第一个指标），下面列同账户其余窗口；不够就用其他账户的最紧张值补到三行 |
| 中 | 每个账户还剩多少 | 一行一账户，窗口横排，最多 6 个账户；一个账户超过 3 个窗口就换行续接，不截、不挑 |
| 大 | 每个窗口分别还剩多少 | 树形：账户名一行，窗口各占一行带进度条 |
| 锁屏矩形 | 最紧张的那个 | 账户名 + 窗口 + 数值 + 倒计时 |

排版逻辑在 `app/layout.ts`，不依赖 Scripting 运行时，所以 `dev/design/impl.png`
这张效果图和 `dev/test_logic.mjs` 里的列宽断言跑的都是它。
