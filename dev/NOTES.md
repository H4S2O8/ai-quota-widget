...
## refresh token 不能被两个客户端共用（这才是登出的原因）
...
**机制**：OAuth 的 refresh token 每次使用都会**轮换**——用掉 A，签发 B，作废 A。
旧的 A 再被用一次，会被判定为**重放**（正常客户端不会这么做，所以推定 token 被盗），
于是整个 token 家族一起撤销。两边都登出。

把 CLI 的 refresh token 复制到手机上，就是让两个客户端共用一个家族。谁先续，
另一个手里的就成了旧的；另一个再续，就触发重放检测。

**这也是之前那次登出更可能的真正原因**，比我说的「限流导致续期失败」更贴合。
当时我的代码还有个 bug：小组件换到新 token 却不保存，于是**每次渲染都重放同一个
旧 refresh token**——那是最标准的重放模式，被撤销几乎是必然的。我当时把原因归到
自己的压测上，那件事确实做错了、也确实打出了 429，但登出这一项，更大的责任在
这个重放 bug。

**先纠正一个说过头的判断**：我曾说共用 refresh token 会「撤销整个家族、两边一起登出」。
实测不是——用户的 Mac 上 CLI 一直正常，只有手机那份报 invalidated。真实机制更简单：

**电脑上的 CLI 每续期一次就轮换一次，把手机上那份副本变成旧的。** 不是撤销，是过期。
所以症状是「手机单方面失效，电脑照常」，而不是两边一起挂。

这也意味着共用是**可用但需要偶尔重同步**的，不是绝对不能用。既然如此，
关键就变成了：**重同步这件事必须足够便宜。** 手打三个长字符串没人会做，
于是加了「粘贴凭据」——电脑上复制整段凭据 JSON，手机上一点全填好。
`extractCredentials()` 按归一化的键名在整棵树里找，各家的键名和层级都不一样。

**两个会造成重放的 bug，都是我的：**

1. **小组件换到新 token 却不保存**（v0.5.0–0.5.4）。每次渲染都拿同一个旧 refresh
   token 去换，这是最标准的重放模式。v0.5.5 修了。
2. **主 App 拿内存里的旧配置去抓**（到 v0.6.0 为止）。小组件是另一个进程，
   它可能刚轮换过 token 并写回磁盘；主 App 手里那份就成了旧的，再用一次就是重放。
   v0.6.1 改成抓之前重读配置和快照，代价只是两次文件读取。

**两个进程共享带轮换的凭据时，「谁手上的副本是最新的」必须每次现问，不能缓存。**

**结论：access token only 才是安全默认。** 两个 provider 的字段说明都改了：
access token 排前面、标为推荐；refresh token 标成「可选，有代价」并说明代价是什么。
经常用 CLI 的人不该填它。

一般化的教训：**复制一份长期凭据到第二个设备，不等于多了一份凭据，等于两个客户端
抢同一份。** 带轮换的凭据尤其如此——共用的结果不是「两边都能用」，是「两边都用不了」。
...
用户反馈：Codex 报「失败 200」，后面跟着一段 HTML，看不懂、复制不了、还被截断。
三个问题叠在一起，等于这个界面在排错上完全没用——而它存在的唯一理由就是排错。

改了三处：

- **能复制**：错误信息、原始响应都给了复制按钮（`Pasteboard.setString`，
  旧的 `Clipboard.copyText` 作兜底）。主列表的错误行、详情页、诊断页都有。
- **不截断到看不懂**：原始响应上限从 4000 提到 20000，并显示实际字符数。
  Cloudflare 验证页开头几千字符全是样式和脚本，真正说明问题的话在后面——
  4000 正好把它切掉。
- **HTML 要认出来**：`looksLikeHtml()`。一个返回 JSON 的接口忽然回 HTML，
  几乎总是同一件事：中间有东西截胡了（Cloudflare 验证页、登录跳转、网络门户）。
  这时候报「HTTP 200」是最误导人的说法——状态码确实是 200，问题在内容。

判据：**排错界面上的每一条信息，都要能被原样带出去。** 看不懂是正常的，
看不懂又带不走就是设计失败。

## 平台全局对象几乎都是异步的，shim 里要写真实签名

`Pasteboard.getString()` 返回的是 `Promise<string | null>`，我当同步值用了，
拿到的是字符串 `"[object Promise]"`。于是「粘贴凭据」永远解析失败，
而且报的是「剪贴板里不是能识别的 JSON」——把人往完全错误的方向指。

类型检查本来能拦，但当时 shim 里 `Pasteboard` 是 any。**给用到的全局写上真实签名之后，
tsc 立刻点名了三处漏掉的 await。** 所以 `dev/typecheck/scripting.d.ts` 的原则改成：
**用到哪个全局，就把哪个的签名补上，不要图省事一律 any。**

顺带：废弃的 `Clipboard` 兜底删掉了。它的名字和 DOM 内置类型撞车，留着会让类型检查失真，
而文档明说 `Pasteboard` 才是现行 API。

另外 iOS 侧有个权限：**设置 → Scripting → 从其他 App 粘贴 → 允许**。
没开的话读剪贴板会失败，所以读不到内容时的提示里写了这条路径。
...
9. **小组件的行高与列宽。** 列宽是从 SF Mono 字体文件量出来的（0.618 em，11pt 下中尺寸
   放 49 列，预算 48）；行高只是按 15–16pt 估的。看两件事：任何一行末尾有没有「…」
   （列宽估多了），中尺寸底部状态行下面是不是空出一大块（行高估多了，`layout.ts` 的
   `ROWS.medium` 可以从 6 提到 7）。
